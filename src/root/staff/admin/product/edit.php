...
WHERE product.ProductId = '$id'";
    ...
    <form method="POST" action="editHandler.php">
        ...
        <input type="text" name="ProductName"
               id="ProductName" value="<?= $product['ProductName'] ?>"><br><br>
        ...
        <select name="Brand" id="Brand">
            ...
            while ($row = $qresult->fetch_assoc()) {
                if ($product['BrandId'] == $row['BrandId']) {
        ...
        <select name="Category" id="Category">
            ...
            $qresult = $connection->query($query);
            while ($row = $qresult->fetch_assoc()) {
                if ($product['SubCategoryId'] == $row['SubCategoryId']) {
        ...
        <input value="<?= $product['ProductCurrentPrice'] ?>" type="number" name="Price" id="Price" min="0" step="0.01">
        <br><br>


        <label for="stock">Stock amount</label> <br>
        <input type="number" name="stock" id="stock" min="0" step="1" value="<?= $product['ProductStock'] ?>"> <br><br>

        <!--    <label for="file">Product image</label><br>-->
        <!--    <input type="file" name="file" id="file"><br><br>-->

        <label for="DescNl">Description (Dutch)</label><br>
        <textarea name="DescNl" id="DescNl" cols="30" rows="10"><?php
            $query = "SELECT * FROM productdescr WHERE LanguageId = 1 AND ProductId = '$id'";
            $qresult = $connection->query($query);
        ...
        <textarea name="DescEn" id="DescEn" cols="30" rows="10"><?php
            $query = "SELECT * FROM productdescr WHERE LanguageId = 2 AND ProductId = '$id'";
            $qresult = $connection->query($query);
            $row = $qresult->fetch_assoc();
            echo $row['ProductDescription'];
            ?></textarea> <br><br>
        ...
        <textarea name="DescFr" id="DescFr" cols="30" rows="10"><?php
            $query = "SELECT * FROM productdescr WHERE LanguageId = 3 AND ProductId = '$id'";
            $qresult = $connection->query($query);
            $row = $qresult->fetch_assoc();
            echo $row['ProductDescription'];
            ?></textarea> <br><br>
        ...
        <input type="text" name="image" id="image" value="<?= $product['ProductImg'] ?>"> <br><br>

        <label for="sku">Stock Keeping Unit [SKU] (Optional)</label><br>
        <input type="text" name="sku" id="sku" value="<?= $product['ProductSku'] ?>"> <br><br>


        <label for="upc">Universal Product Code [UPC] (Optional)</label><br>
        <input type="number" name="upc" id="upc" value="<?= $product['ProductUpc'] ?>"><br><br>

        <label for="visible">Product Is Visible?</label><BR>
        <input type="checkbox" name="visible" id="visible" <?php
        if ($product['ProductIsVisible'] == 1) {
            echo "checked";
        }
        ?>><br><br>

        <input type="text" name="ProductId" id="ProductId" value="<?= $id ?>" hidden>
